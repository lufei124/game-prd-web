...
// command to change workflow, grant confirmation, or silently select extensions.
export function conversationOptions(s: Store, requirementId: string, body: any) {
  ...
  if (body.chatOnly) return { ...body, kind: "requirement", executor: "codex", conversation: false, chatOnly: true };
  const stage = body.stage || new Domain(s).conversationStage(r);
  const action = body.action || "discuss";
  const kind = stage === "review" && action !== "generate" && r.heads.review ? "prd" : stage;
  return {
    ...body,
    kind,
    stage,
