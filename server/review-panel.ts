...
function requestedRoles(input: AgentInput) {
  const marker = input.prompt.match(/REVIEW_ROLES_JSON:(\[[^\n]+\])/);
  if (!marker) return input.snapshot.reviewRoles || reviewRoles;
  try {
    ...
    /* Fall back to the frozen defaults below. */
  }
  return input.snapshot.reviewRoles || reviewRoles;
}
...
export async function runReviewPanel(
    ...
    await runtime.run(
      ...
      },
      {
        ...
        read: async (name, args) => {
          ...
          check(
            name !== "ask_question",
            "评审中的待澄清问题请列入评审 issues",
          );
          check(
            ["read_context", "read_resource", "read_knowledge"].includes(name),
            "评审只允许读取冻结上下文",
            403,
          );
  ...
  const metadata = {
    summary: reports
      .map(({ role, report }) => `${role.name}：${report.summary}`)
      .join("\n"),
    issues: reports.flatMap(({ role, report }) =>
      report.issues.map((issue: any, i: number) => ({
        ...issue,
        id: `${role.id}-${i + 1}`,
        sourceRole: role.name,
        description: `【${role.name}】${issue.description}`,
      })),
    ),
  };
  const summary = `已完成 ${reports.length} 个独立视角评审，共发现 ${metadata.issues.length} 个问题。请逐条选择采纳、不采纳或稍后处理，再由主助手统一修改 PRD。`;
