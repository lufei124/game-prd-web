...
test("multi-turn conversation uses built-in stages, preserves questions and updates the requirement draft (Mock)", async () => {
  const root = await mkdtemp(join(tmpdir(), "conversation-")), s = new Store(root);
  await bootstrap(s);
  const d = new Domain(s), p = d.project("产品"), r = d.requirement(p.id, "登录");
  let turns = 0;
  const runtime = {
    async run(input: any, host: any) {
      ...
      assert(input.snapshot.messages.some((m: any) => m.content === "面向哪些用户？"));
      assert(input.snapshot.questions.some((q: any) => q.answer === "企业管理员"));
  ...
  try {
    ...
    assert.equal(first.snapshot.releases.length, 0);
    ...
    assert.equal(conversationOptions(s, r.id, { prompt: "使用墨色风格生成原型" }).kind, "requirement");
