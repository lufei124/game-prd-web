...
test("conversation stage is explicit; keywords and legacy style bindings do not switch workflow", async () => {
  const root = await mkdtemp(join(tmpdir(), "workflow-"));
  const s = new Store(root);
  try {
    await bootstrap(s);
    const d = new Domain(s), p = d.project("产品"), r = d.requirement(p.id, "需求");
    const o = conversationOptions(s, r.id, {
      prompt: "PRD 和原型是什么意思？切换墨色风格",
      stage: "requirement",
    });
    assert.equal(o.kind, "requirement");
    assert.equal(o.styleId, undefined);
    assert.equal(o.executor, "codex");
    ...
    assert.equal(
      conversationOptions(s, r.id, { prompt: "修改按钮操作", stage: "prototype" }).scope,
      "layout",
    );
    assert.equal(
      conversationOptions(s, r.id, { prompt: "补充需求卡", stage: "requirement" }).kind,
      "requirement",
    );
    const style = new Extensions(s).list().find((e) => e.name === "低保真线框风格")!;
    s.put("requirement", {
      ...s.get("requirement", r.id),
      assistantDefaults: { styleId: style.id },
    });
    ...
    const t = tasks.create(
      r.id,
      conversationOptions(s, r.id, {
        prompt: "讨论原型",
        stage: "prototype",
      }),
    );
    assert.equal(
      t.snapshot.releases.filter((x: any) => x.manifest.type === "style").length,
      0,
    );
...
test("review uses three isolated calls and publishes one aggregate; partial failures publish nothing (Mock)", async () => {
  ...
  const input: any = {
    ...
    kind: "review",
    prompt: "评审",
    snapshot: { reviewRoles, heads: { prd: "frozen" } },
  ...
  assert.equal(
    new Set(outputs[0].args.metadata.issues.map((i: any) => i.id)).size,
    3,
  );
  outputs.length = 0;
...
test("default upgrade backs up data and appends releases while preserving user edits", async () => {
  ...
  try {
    ...
    const previous = {
      ...
      files: {
        ...old.files,
        [old.main]: Buffer.from("old instructions").toString("base64"),
      },
      source: "builtin:game-prd-web/requirement@1",
    };
    s.put("release", previous);
    const prototype = s
      .all("extension")
      .find((e) => e.builtinKey === "prototype");
    const protoRelease = s.get("release", prototype.currentRelease);
    const user = ex.install(
      protoRelease.files,
      "local-editor",
      protoRelease.manifest,
      prototype.id,
    );
    s.put("settings", {
      ...s.get("settings", "system"),
