...
- 61 项单元测试、18 项完整 Chrome E2E、构建及差异检查通过。
- 目录选择通过显式 Mock 验证关联成功与取消无新增；HTTP 测试验证缺失 CSRF 时不会调用选择器。系统窗口本身待桌面人工选择验证，Windows/Linux 待对应平台实测。
- 飞书使用页面居中 dialog，验证居中坐标、取消、空输入禁用和成功关联同步。真实飞书连接仍取决于本机配置，测试使用 Mock。
- 本地 4317 服务已重启加载新接口。
